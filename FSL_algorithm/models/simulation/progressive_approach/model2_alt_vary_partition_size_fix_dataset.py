...
def run_model(device, dataloaders, data, constant):
    ...
    wd = os.path.join(constant.PD, 'm2_alt_'+str(constant.PARAM)+'_reconstruction_client_'+str(constant.CLIENTS)+"_vary_partition_size_fix_dataset_base_"+str(constant.MAXCLIENTS)+"_"+str(constant.CUTS[1])+"_"+str(data)+"_"+constant.DATA_DIST+"_SerAvg")
    ...
    ##############################################################
                for index in range(constant.PARAM):
                    forward_clients, backward_clients, intermediate, steptime_client = trainA(images, splitNNs['splitNN{}'.format(loader_cli_map[dat_idx][idx%len(loader_cli_map[dat_idx])]+1)])
                    total_steptime_client_trainA += steptime_client[0]
