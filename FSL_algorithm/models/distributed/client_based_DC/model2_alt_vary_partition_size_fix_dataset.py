...
def trainA(x, splitNN):
    ...
    intermediate = splitNN.forwardA(x)
    ...
    return forward_clients+backward_clients, intermediate

def run_model(device, dataloaders, data, constant):
    if(torch.cuda.is_available()== True):
        torch.cuda.reset_max_memory_allocated()
    wd = os.path.join(constant.PD, "m2_alt_"+str(constant.PARAM)+"_reconstruction_vary_partition_size_fix_dataset_"+str(constant.CLIENTS)+"_base_"+str(constant.MAXCLIENTS))+"_dist"
    ...
    while(epoch < constant.EPOCHS):
        ...
        for filling_data in range(math.ceil(constant.CLIENTS/constant.CLIENTS)):
            for idx, (images, labels) in enumerate(dataloaders['train']):
                ...
                for index in range(constant.PARAM):
                    end_clients, intermediate = trainA(images, splitNNs['splitNN{}'.format((j%constant.CLIENTS)+1)])
                    total_time_client += end_clients
        ##############################################################
        ...
        ##############################################################
            ...
            with torch.no_grad():
                ...
                #Validation
                for k in range(constant.CLIENTS):
                    models['models{}'.format(k+1)][1].send(client_array[k][1])
