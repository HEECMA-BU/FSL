...
def trainA(x, splitNN, ready_clis_l):
    splitNN.zero_grads()
    ...
    splitNN.backwardA_NoPeek(ready_clis_l)
    backward_clients = time.time() - begin2_clients

    client_step_time = splitNN.stepA(ready_clis_l)
...
def run_model(device, dataloaders, data, constant):
    ...
    wd = os.path.join(constant.PD, 'm1_alt_'+str(constant.PARAM)+'_reconstruction_client_'+str(constant.CLIENTS)+"_vary_partition_size_fix_dataset_base_"+str(constant.MAXCLIENTS)+"_"+str(constant.CUTS[1])+"_"+str(data)+"_"+constant.DATA_DIST)
...
############################################################## Save Intermediate Before DC Round
                # # save_time = time.time()
                ...
                #     # # since += save_time
                #     del intermediate
############################################################## Run DC Round and Save Corresponding Intermediate/Client Output(not sent on wire)    
                for index in range(constant.PARAM):
                    forward_clients, backward_clients, client_step_time, intermediate_list = trainA(images_array, splitNN, ready_clis_l)
                    for idx, (intermediate, images, labels) in enumerate(zip(intermediate_list, images_array, labels_array)):
                        intermediate = intermediate.get()
                        # if index == constant.PARAM-1:
                        if batch_count <= 101:
                            torch.save(intermediate,   alt_intermediate_logpath_l[index]+str(epoch)+"_"+str(batch_count)+'_Client'+str(idx)+'.pt')
                            torch.save(labels,         alt_label_logpath_l[index]+str(epoch)+"_"+str(batch_count)+'_Client'+str(idx)+'.pt')
                        del intermediate      

                    total_steptime_client_trainA += client_step_time
                    total_time_client_trainA += forward_clients+backward_clients
############################################################## Clean Up for Next Batch
                for idx, (images, labels) in enumerate(zip(images_array, labels_array)):
                    images = images.get()
                    # labels = labels.get()
                    del images
                    del labels
