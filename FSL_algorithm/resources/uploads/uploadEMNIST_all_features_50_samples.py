...
def uploadEMNIST(seed_set, device, batch_size):    
    ...
    trainset = torch.utils.data.Subset(trainset, range(0,50))
    valset = torch.utils.data.Subset(valset, range(0,50))
    ...
    valloader = torch.utils.data.DataLoader(valset, batch_size = batch_size, shuffle=True, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"))
