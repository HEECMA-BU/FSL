...
def uploadMNIST(seed_set, device, batch_size, flag):    
    ...
    if(flag == 'diffDistrmodel1'):
        ...
        #trainloader = torch.utils.data.DataLoader(mnist, batch_size=batch_size,sampler = sampler,shuffle=False, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"), drop_last=True)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,sampler = sampler, shuffle=False, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"), drop_last=True)
        ...
        mask = torch.tensor(mask)   
        sampler = YourSampler(trainset, mask)
        trainloader1 = torch.utils.data.DataLoader(trainset, batch_size=batch_size,sampler = sampler, shuffle=False, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"), drop_last=True)
        ...
        #trainloader = torch.utils.data.DataLoader(mnist, batch_size=batch_size,sampler = sampler,shuffle=False, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"), drop_last=True)
        valloader = torch.utils.data.DataLoader(valset, batch_size=batch_size,sampler = sampler1, shuffle=False, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"))
        mask1 = [1 if valset[i][1]%2 == 1 else 0 for i in range(len(valset))]
        mask1 = torch.tensor(mask1)   
        sampler1 = YourSampler(valset, mask1)
        valloader1 = torch.utils.data.DataLoader(valset, batch_size=batch_size,sampler = sampler1, shuffle=False, worker_init_fn=seed_set, pin_memory=(device.type == "cuda"))    
        return trainloader, trainloader1, valloader, valloader1
    else:
