...
from FSL_algorithm.resources.setup import average_weights_4, setup2, average_weights, setup3_4
...
def train(x, target, model, train_shared):

    model.zero_grads()
    model.set_parameter_requires_grad(train_shared)
    ...
    step_times = model.step_4(train_shared)  

    if train_shared: 
        return loss, pred, 0, 0, forward_time, backward_time, step_times, intermediate
    else:
...
def run_model(device, dataloaders, data, constant):  # layers in between CUTS is corresponding to the client in FSL
    ...
    wd = os.path.join(constant.PD, 'm4_nop_reconstruction_client_'+str(constant.CLIENTS)+"_vary_partition_size_fix_dataset_base_"+str(constant.MAXCLIENTS)+"_"+str(constant.CUTS[1])+"_"+str(data)+"_"+constant.DATA_DIST)
    ...
    for i in range(constant.CLIENTS):
        # for i in range(constant.THOR):
            ...
            for model in models['models{}'.format(i+1)]:
                seqs.append(nn.Sequential(*(model[:constant.CUTS[1]])))
                seqs.append(nn.Sequential(*(model[constant.CUTS[1]:])))
            ...
            optimizers['optimizer{}'.format(i+1)] =  [
                ...
                for model in seqs]
            scheduler['optimizer{}'.format(i+1)] = [ExponentialLR(opt_init, gamma=0.9) for opt_init in optimizers['optimizer{}'.format(i+1)]]   #TODO: check number of optimizors and models(only 1 in each client)
    ...
    while(epoch < constant.EPOCHS):
        ...
        if (epoch == 0):
            for k in range(constant.CLIENTS):
                ...
                seqs = []
                for model in models['models{}'.format(k+1)]:
                    seqs.append(nn.Sequential(*(model[:constant.CUTS[1]])))
                    seqs.append(nn.Sequential(*(model[constant.CUTS[1]:])))
                for i in range(len(seqs)):
                    optimizer1 = optim.AdamW(seqs[i].parameters(), lr=constant.LR, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01, amsgrad=False)
        ...
        for dat_idx in range(len(dataloaders['train'])):
            for idx, (images, labels) in enumerate(dataloaders['train'][dat_idx]):
                ...
                for train_shared in [False, True]:
                    loss, output, end_clients_forw, end_clients_back, end_server_forw, end_server_back, step_times, intermediate = train(images, labels, splitNNs['splitNN{}'.format(loader_cli_map[dat_idx][idx%len(loader_cli_map[dat_idx])]+1)], train_shared)
                    temp = loss.get()
                    running_loss[idx%constant.CLIENTS] += float(temp)
                    labels_local = labels.copy().get()
                    output = output.get()

                    # pred, target_tot, pred_tot =  make_prediction(output, labels_local.tolist(), target_tot, pred_tot)
                    target = labels_local.tolist()
                    ...
                    if train_shared:
                        total_steptime_server += step_times[0]  # have just trained the corresponding server part in FSL
        ...
        ##############################################################
                    ...
                    #     torch.save(labels_local,         path3+str(epoch)+'_Client_'+str(dat_idx)+"_"+str(idx)+'.pt')
        ...
        ##############################################################
            ...
            if data == 'covid':
                ...
                # logger.debug(cm1)
                for i in range(constant.CLIENTS):
                    logger.debug('Phase: {} Epoch: {} Loss: {:.4f} F1_Score {:.4f}'.format('train', epoch, running_loss[i%CLIENTS]/lun[i%CLIENTS], f1_score_value))

            with torch.no_grad():
                #Average weights
                averaging_time_client = time.time()
                average_weights_4(models, local_models, False, constant, 0)    # thor=0: pick the whole model in client, and then average the shared part
