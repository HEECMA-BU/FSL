###################################################################################################
#Parallel Split Learning with Differential Privacy
###################################################################################################
import logging
import math
import torch
...
def train(x, target, splitNN, batch_size, ready_clis_l):
    ...
    client_step_time, server_step_time = splitNN.step(ready_clis_l)
...
def run_model(device, dataloaders, data, constant):
    ...
    wd = os.path.join(constant.PD, 'm1_dp_'+str(constant.PARAM)+'_reconstruction_client_'+str(constant.CLIENTS)+"_vary_partition_size_fix_dataset_base_"+str(constant.MAXCLIENTS)+"_"+str(constant.CUTS[1])+"_"+str(data)+"_"+constant.DATA_DIST)
