...
def trainA(x, splitNN):
    ...
    return forward_clients+backward_clients, intermediate_list 

def run_model(device, dataloaders, data, constant):
    if(torch.cuda.is_available()== True):
        torch.cuda.reset_max_memory_allocated()
    wd = os.path.join(constant.PD, "m1_alt_"+str(constant.PARAM)+"_reconstruction"+"_vary_partition_size_fix_dataset_"+str(constant.CLIENTS)+"_base_"+str(constant.MAXCLIENTS))+"_dist"
                ...
                for images, labels in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        if (j%constant.CLIENTS == 0 and phase == 'train'):
                            ...
                            for index in range(constant.PARAM):
                                end_clients, intermediate_list = trainA(images_array, splitNN)
        ##############################################################                        
